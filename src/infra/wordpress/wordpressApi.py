...
class WordpressApi(object):
    ...
    def __init__(self, config: dict[str:str], requestsProxies=None):
        self.configData = config
        self.host = config["host"]  # 'https://www.crifan.org'
        self.authorization = None  # 'Bearer xxx'
        ...
        self.apiAuthToken = self.host + "/wp-json/jwt-auth/v1/token"
    ...
    def loginWp(self):
        ...
        try:
            ...
            response = requests.post(self.apiAuthToken, data=credentials)

            # Mengecek status kode
            if response.status_code == 200:
                # Mendapatkan token dari respon JSON
                data = response.json()
                token = data.get("token")
                print("Login berhasil!")
                print(f"Token: {token}")
                self.authorization = "Bearer %s" % token
            else:
    ...
    def validateToken(self):
        ...
        isTokenOk, respInfo = WordpressApi.processCommonResponse(resp)
        if isTokenOk:
            respCode = respInfo["code"]
            if respCode == WordpressApi.JWT_TOKEN_VALID_CODE:
                isTokenOk = True
            else:
                isTokenOk = False
    ...
    def createMedia(self, contentType, filename, mediaBytes):
        ...
        isUploadOk, respInfo = WordpressApi.processCommonResponse(resp)
    ...
    def createTaxonomy(self, name, taxonomy, parent=None, slug=None, description=None):
        ...
        isCreateOk, respInfo = WordpressApi.processCommonResponse(resp)
    ...
    def getTaxonomySinglePage(self, name, taxonomy, curPage, perPage=None):
        ...
        if perPage is None:
            perPage = WordpressApi.SearchTagPerPage
        ...
        isSearchOk, respTaxonomyLit = WordpressApi.processCommonResponse(resp)
    ...
    def searchTaxonomy(self, name, taxonomy):
        ...
        if isGetAllOk:
            ...
            finalRespTaxonomy = WordpressApi.findSameNameTaxonomy(
